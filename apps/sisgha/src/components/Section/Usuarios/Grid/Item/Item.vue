...
const perfisQuery = useQuery({
  queryKey: ["usuarios", computed(() => `usuario::id::${unref(usuario.value.id)}`), "usuarios::perfis"],
  queryFn: () => {
...
const perfisConcatenated = computed(() => {
  const allPerfisLabels = perfis.value.map((perfil) => getRoleLabel(perfil.cargo));
...
const profilePicureUrl = useApiImageRoute(ApiImageResource.USUARIO_PROFILE, usuario);
</script>
      ...
      >
        <template v-slot:placeholder>
          <div class="d-flex align-center justify-center fill-height">
            <v-progress-circular indeterminate v-if="Boolean(profilePicureUrl)" />
        ...
        <template v-slot:error>
          <div class="d-flex align-center justify-center fill-height">
            <v-empty-state icon="mdi-alert-circle-outline" text="Não foi possível buscar esta foto" />
          </div>
        </template>
      </v-img>
