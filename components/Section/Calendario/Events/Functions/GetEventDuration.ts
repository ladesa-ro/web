...
async function startEvent(date1: dayjs.Dayjs, date2: dayjs.Dayjs): Promise<string> {
  try {
    const diff = date1.diff(dayjs().toDate());

    // Verify if event on started
    if (diff <= 0) return endEvent(date2);
    else {
...
async function endEvent(date: dayjs.Dayjs): Promise<string> {
  try {
