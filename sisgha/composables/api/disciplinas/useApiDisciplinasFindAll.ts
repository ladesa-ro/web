...
export const useApiDisciplinasFindAll = async (
  ...
  //busca no BD para uma consulta

  const apiClient = useApiClient();

  const query = useQuery({
    //Cache= Salvamento local de navegador --> Algo FIXO
    ...
    queryFn: async () => {
      //função que realmente busca os dados cadastrados, e obtém uma função de busca recente em uma página.
      return apiClient.disciplinas.disciplinaList({
